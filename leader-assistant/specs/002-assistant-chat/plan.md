...
- [x] **P1 Vault is source of truth** — conversations persist to `sessions/`; answers come
  from vault content; no separate chat datastore. ([[03-vault]], [[06-conversations]])
- [x] **P2 `raw/` immutable** — the agent's write tools are routed through the existing
  `vault.guard_write_path`; `raw/` is read-only. ([[18-security]])
- [x] **P3 Pipeline direction** — chat writes only to `sessions/`; promotion to `wiki/`
...
- [x] **P13 Multi-vault** — the conversation resolves a vault via the existing resolver;
...
- **Agent runtime:** `claude-agent-sdk` (already a dependency) — `include_partial_messages`
  ...
  `create_vault`, `spec_read`, …), not raw filesystem access (D3). This mirrors the proven
...
- **Conversation store (durable):** the vault's `sessions/` is the **source of truth** for a
...
- **Dependencies on other features:** builds directly on feature 001's `app/capabilities`
  (`query`, `plan`, `ingest`, `spec_read`, `list/create/get vault`) and `app/vault`
...
(future GUI)                 ┘── app/capabilities.ask() ──► agent runtime (claude-agent-sdk)
                                        │                        │ tools = capabilities
                                        │                        ▼
                                        │             query · plan · spec_read · create_vault
                                        ▼                        │  (all governed)
...
- `ask()` owns: resolve the conversation (load prior turns from `sessions/` by
  `conversation_id`), resolve the vault, load the persona prompt, run the agent with
  **capabilities as its tools**, classify consequential vs. routine, and append the turn to
  the conversation's session file.
- The agent browses knowledge **only via `query`** (cited); it never reads vault files
...
- **`capabilities.ask(vault, message, conversation_id) -> ChatAnswer`** — surface-agnostic
  ...
  - reads: conversation from `sessions/`; vault via resolver; persona from `memory/` + `specs/`.
...
- **Agent tool adapter** — exposes **capabilities as the agent's tools**: `query` (the only
  knowledge-browsing tool, D3), `plan`, `spec_read`, `create_vault`, and (gated) `ingest`.
  No raw `Read/Glob/Grep` browse tool. Every writing capability enforces
  `vault.guard_write_path`. (FR-2, FR-5, FR-9, FR-10, FR-11)
- **Conversation store** — durable record under `<vault>/sessions/`, keyed by
...
- **Chat request** (both endpoints):
  ```json
  { "message": "string", "vault": "optional-selector", "conversation_id": "optional-id",
    "approve": false }
  ```
...
- **Chat response** (`/api/chat`):
  ```json
  { "vault": "name", "conversation_id": "id", "reply": "string",
    "citations": [{"page": "wiki/...md", "excerpt": "..."}],
...
- **Session file (durable conversation record)**: `<vault>/sessions/<conversation_id>.md`
...
- `ask(vault, message, conversation_id, approve=False) -> ChatAnswer` — non-streaming.
- `ask_stream(vault, message, conversation_id, approve=False) -> AsyncIterator[ChatDelta]` — streaming.
...
- **MVP:** `ask()` (non-streaming) with the Product Owner persona and `query` as the
  ...
  `conversation_id`); default-vault resolution. Consequential requests return a
...
- **Later:** add `create_vault` and gated `ingest`/`produce` as agent tools; optional
