...
- [x] T011 [P] Persona guardrails in the prompt: answer from the vault with citations,
  plan-first for consequential work, state assumptions, never write `raw/`. (FR-2, FR-5, FR-11, FR-12)

## Core — conversation store (durable, resumable)

- [x] T015 Conversation store: `<vault>/sessions/<conversation_id>.md` — one file per
...
- [x] T020 `capabilities.ask(vault, message, conversation_id, approve=False) -> ChatAnswer`:
  load/create the conversation, resolve vault, load persona, run the agent, append the turn,
...
- [x] T021 Agent tool adapter: expose **capabilities as tools** — `query` as the **only**
  knowledge-browsing tool (D3), plus `plan`, `spec_read`, `create_vault`. No raw
...
- [x] T025 Vault resolution: default when omitted; **create only on explicit user request**
  via `create_vault` tool; report a missing/misspelled named vault instead of creating it. (FR-10; P13; D1)
...
- **D1** — chat may create a vault only on explicit user request, via the `create_vault`
