...
- [x] **P1 Vault is source of truth** — the UI holds only transient view state
  (`conversation_id`, active vault); the durable conversation lives in `sessions/` on the
  server. ([[03-vault]], [[06-conversations]])
- [x] **P2 `raw/` immutable** — the UI issues no writes except through API calls that already
  route through `vault.guard_write_path`. ([[18-security]])
- [x] **P3 Pipeline direction** — unaffected; the UI triggers nothing that writes to `wiki/`
...
- [x] **P9 Interface parity** — the UI is a **surface over the REST API**; it adds no
  capability and reaches the vault only via `/api/*`. ([[12-assistant]], [[13-api]], [[14-chat]])
...
- [x] **P13 Multi-vault** — a vault picker selects the active vault; default when none chosen.
  ([[03-vault]])

## Technical Context

- **Language / runtime:** Python 3.13+ (matches `app/`).
- **UI framework:** `gradio` — `gr.Blocks` for the chat + vault-picker layout, mounted onto
...
- **API client (UI → API):** an HTTP client (e.g. `httpx.AsyncClient`) issuing same-origin
  calls to `/api/chat/stream` (SSE), `/api/chat`, and `/api/vaults`. Streaming is consumed
...
- **Docs relocation:** configure FastAPI with `docs_url="/api"` so Swagger UI serves at
  ...
  (Swagger) does not shadow `/api/vaults` etc. (FR-2). Decide alongside whether `openapi.json`
...
- **Dependencies on other features:** consumes feature 002 endpoints (`/api/chat`,
  `/api/chat/stream`) and feature 001 vault endpoints (`/api/vaults`). Adds no capability.
...
Browser
  ...
  ▼
Gradio UI process ──HTTP(same origin)──► /api/vaults        (list / create)
  (app/ui.py)                            /api/chat           (full reply)
                                         /api/chat/stream    (SSE, incremental)
                                              │
                                              ▼
                                   app/capabilities.ask()  ──► agent ──► app/vault ──► vault
                                   (feature 002 — unchanged)
```

- The UI is thin: capture input + view state (`conversation_id`, active vault) → call the
...
- **`app/ui.py` — Gradio UI builder.** `build_demo()` returns a `gr.Blocks` app with: a
  title, a vault picker (dropdown from `GET /api/vaults` + a "create vault" control calling
  `POST /api/vaults`), a `gr.Chatbot`, an input textbox, and `gr.State` for `conversation_id`
  and active vault. Handlers call the API client and stream replies. Also exposes a
  pending-plan panel + approval control. (FR-1, FR-4, FR-5, FR-6, FR-8, FR-9)
- **API client helpers (in `app/ui.py`).** `stream_chat(vault, message, conversation_id,
  approve)` consumes `/api/chat/stream` and yields accumulated reply + `conversation_id`
  (+ `pending_plan`/`citations`); `chat(...)` for the non-streaming fallback; `list_vaults()`
  / `create_vault(name)` over `/api/vaults`. All same-origin HTTP (D3). (FR-3, FR-4, FR-6, FR-11)
...
and the feature 001 vault contract.

- **Chat call (UI → API):** `POST /api/chat/stream` (SSE) or `POST /api/chat` with
  ```json
  { "message": "string", "vault": "active-or-null",
...
- **Vault calls (UI → API):** `GET /api/vaults` → `{root, vaults[], default}` populates the
  picker; `POST /api/vaults {name}` creates a vault on explicit user action (FR-6).
- **UI view state (transient, non-persisted):** `conversation_id` and active vault in
  `gr.State`; the durable record remains `sessions/<id>.md` on the server (P1).
...
- **MVP:** relocate Swagger to `/api/` and mount a Gradio chat UI at `/`; single message →
  ...
  default-vault chat. Update the startup banner.
- **Then:** vault picker (list/select via `/api/vaults`, create on explicit action); citations
