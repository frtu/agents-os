...
> no dependency). Consumes feature 002 chat endpoints and feature 001 vault endpoints; adds
> **no** capability (P9).
...
- [ ] T020 `app/ui.py` API client helpers over HTTP only: `list_vaults()` / `create_vault(name)`
  (`/api/vaults`), `chat(...)` (`/api/chat`), `stream_chat(...)` (`/api/chat/stream`, SSE).
...
- [ ] T022 Error mapping: turn non-2xx API responses (e.g. missing named vault) into a
  visible UI message. (FR-11; AC-9)

## UI — chat surface (startup)

- [ ] T030 `build_demo()` Gradio `gr.Blocks`: title, `gr.Chatbot`, input textbox, and
  `gr.State` for `conversation_id` and active vault. (FR-1)
...
- [ ] T040 Vault dropdown populated from `GET /api/vaults`; selecting sets the active vault in
  state; show the active vault to the user. (FR-6, FR-7)
- [ ] T041 "Create vault" control → `POST /api/vaults` on explicit user action, then refresh
  the dropdown so the new vault appears. (FR-6; D2)
- [ ] T042 Default-vault behavior when none selected (send `vault: null`; server defaults). (FR-7; P13)
...
- [ ] T061 AC-2: Swagger UI resolves at `/api/` **and** `/api/chat` + `/api/vaults` still
...
- [ ] T064 AC-5: picker lists vaults, switching works, a UI-created vault appears; none
  selected → default vault used. (FR-6, FR-7)
...
- [ ] T067 AC-8: `app/ui.py` imports no `app.capabilities`/`app.vault` and touches no vault
  file directly — verified by inspection/test (UI calls only `/api/*`). (FR-3, FR-10; P9)
- [ ] T068 AC-9: an API error (e.g. missing named vault) is shown in the UI, not swallowed. (FR-11)
...
- **D2** — scope is chat + vault picker only; full capability console deferred. (T030–T051)
