...
The assistant proposes; the human curates. **Consequential work** — anything the Risk Engine flags, external PM actions, and destructive/irreversible changes — MUST NOT execute unless the human is in control. The human is "in control" when **any** of these conditions holds:

- **Per-action review:** the human reviews a concrete plan of the *actual* action before it runs; or
- **Standing consent:** the operator has **explicitly and deliberately** enabled **auto-approve ("trust mode")** — a revocable setting the human alone controls — in which case pre-authorized consequential actions run without a per-action prompt; or
- **Bounded checker:** a non-human **checker** answers the approval on the operator's behalf, **only** within limits the operator sets and *deterministic code* enforces.

A bounded checker is constrained absolutely: it **may never widen its own authority**; it **may never grant** consent for an operation shape lacking **either** standing consent **or** recorded operator precedent; it **MUST fail closed to asking** when unavailable, uncertain, or malformed; and it **MUST NOT be able to execute** anything. A checker may learn to **ask less**; it may never learn to **refuse more** — an autonomous refusal is permitted only on the precedent of a prior operator refusal of the same shape. The grant is issued by code after the checker speaks, never by the checker itself.

**Separation of maker and checker is the whole basis of this delegation.** The party that *performs* the work may request clarification and may request approval, but can **never** grant, answer, or bypass its own approval, and has no means to read or set standing consent. Delegation is permitted only to a checker that cannot act; it is never permitted to the actor.

In **all** cases every executed mutation MUST remain **auditable and revertible**: recorded in `log.md` and committed to the workspace's git repo, so the human can review and undo after the fact — and the record MUST state **which party decided** (operator, standing consent, or checker) and why. Risk is evaluated on the **actual effect** of the operation about to run — its declared effect tier and scoring modifiers (P12), evaluated at the boundary and **accumulated across the whole execution a request causes** — **never** on the wording of a request; and the assistant MUST NOT prompt for an approval it **cannot execute** (no dead-end approvals). Operations that mutate `vault/wiki/` surface their intent and key takeaways for review. Routine autonomous operations (ingestion, dreaming, lint, draft generation) proceed without a plan, recording their effects in portal/log/git.
...
Every proposed mutation is evaluated by an **extensible Risk Engine whose rules are declared as data** (`id, scope, condition, severity, action`), not hard-coded branches. **Rules-as-data covers the scoring model too**: the **risk-scoring modifiers** that raise or lower an operation's score (`name, condition, weight, reason`) are declared as data alongside the effect table, and a modifier MUST describe only the operation's **effect** — never the operator's trust setting, precedent, or the request's wording, which belong to the checker. Safe changes commit to `main`; risky changes go to a `feature/{name}` branch and are **never silently merged** without the approval policy being satisfied. Each evaluation is auditable in `log.md`.
