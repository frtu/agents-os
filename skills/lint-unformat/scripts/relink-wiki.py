...
Usage:
    python scripts/relink-wiki.py <wiki-root> [--dry-run] [--verbose] [--file PATH]

<wiki-root> is a path (relative or absolute) to a wiki directory, e.g. `wiki`
when run from a vault directory. Content pages are read from its standard
subfolders (concepts, product, resources, people, projects, synthesis).
"""
...
# Wiki root directory. Set at runtime from the positional path argument.
...
def build_vocabulary() -> Dict[str, List[str]]:
    ...
    for folder in CONTENT_FOLDERS:
        ...
        for md_file in folder_path.rglob("*.md"):
            ...
            vocab[slug] = generate_variations(slug)
...
def process_file(file_path: Path, vocab: Dict[str, List[str]], dry_run: bool = False, verbose: bool = False) -> int:
    ...
    if verbose:
        try:
            display_path = file_path.relative_to(WIKI_ROOT)
        except ValueError:
...
def main():
    ...
    parser.add_argument("path", nargs="?", default="wiki", help="Wiki root directory to scan (relative or absolute; default: wiki)")
    parser.add_argument("--dry-run", action="store_true", help="Show what would be changed without modifying files")
    parser.add_argument("--verbose", "-v", action="store_true", help="Show detailed output")
    parser.add_argument("--file", type=str, help="Process only a specific file (absolute, or relative to the wiki root)")
    args = parser.parse_args()

    global WIKI_ROOT
    WIKI_ROOT = Path(args.path).expanduser().resolve()
    ...
    if args.file:
        # Process single file (absolute path, or relative to the wiki root)
        ...
        # Process all content folders under the wiki root
        for folder in CONTENT_FOLDERS:
            folder_path = WIKI_ROOT / folder
            if not folder_path.exists():
                continue
