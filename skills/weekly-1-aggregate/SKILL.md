...
**`slack-unformat`** skill before parsing — run it over the raw files for the date:

```bash
python3 .claude/commands/slack-unformat/scripts/normalize_slack_emoji.py "raw/daily/Weekly updates/<date>-<product>/"*.md
```

Use `--dry-run` first to preview. See the `slack-unformat` skill for details.
...
1. Normalize emoji first with the `slack-unformat` skill (see [Emoji Normalization](#emoji-normalization)).
2. Read the file.
